...
sudo apt-get install ipython3 python3-tk python3-pytest
  ...
  openjdk-7-jre libnetcdf-dev libgeos-dev libhdf5-dev
\end{minted}
...
\begin{exercise}
Using the code you wrote in Practical~\ref{ex:mean}, create a complete Python program that has the same behaviour as before.  You should supply the two numbers as command line arguments.
...
Create two new files, each containing a single input number.  Using the \shellcmd{cat} command and a pipe (described in Practical~\ref{ex:sort}), use these files as input to your Python program.
...
Notice that we can use as multiple \shellcmd{elif} statements.
	\item If we need to execute the same code many times, we can use a control structure called a `loop'.  Python offers two loop types: the \shellcmd{while} loop and the \shellcmd{for} loop.  We discuss \shellcmd{for} loops later in section~\ref{sec:lists}.
\index{loop}
...
\end{minted}
\label{blast-off}
We will rewrite this more succinctly using a \shellcmd{for} loop in Practical~\ref{ex:bisection}.
\end{itemize}
...
\end{align*}
\begin{enumerate}
\item Create an array that represents the initial conditions, $\phi^{(0)}$, using $N = 40$.  Let $\phi(x) = \sin(\pi x)$ where $x \in \mathbb{R} | 0 \leq x \leq 1 $.
\item Assuming $K=\num{1e-3}$ and $\Delta t = 0.1$, create the matrix of coefficients and use \shellcmd{scipy.linalg.solve()} to find the value of $\phi^{(1)}$.
...
\begin{exercise}
Plot the initial and final conditions of the diffusion problem in Practical~\ref{ex:diffusion}.

\additional

\begin{enumerate}
	\item For the numeric integration program you wrote in Practical~\ref{ex:integration}, plot the accuracy versus $\Delta x$.  What can you conclude from this plot?
...
\begin{itemize}
	\item Returning to the root finding program from Practical~\ref{ex:bisection}, we might want to reuse the function for finding roots of different functions.  A good way to code this is to pass the function as an argument to the \shellcmd{find\_root()} function:
\begin{minted}{python}
...
\end{itemize}
\begin{exercise}
Modify the numeric integration program from Practical~\ref{ex:bisection} to accept the integrable function as an argument.  Try integrating some named functions and lambda expressions.

\additional

\textit{Note: I do not have my own solution for the following exercise, but I am able to help with any programming problems you have!}

Find the vector, constrained with a specified magnitude, for throwing a stone so that it lands at a specified position.

You will need a function which accepts an input vector argument, described in terms of angle and elevation, and returns the landing point.  Use \shellcmd{scipy.optimize.fmin\_cg()} to minimize the distance between the landing point computed by the function, and the desired landing point.

Alter the model to account for:
\begin{enumerate}
	\item a prescribed prevailing wind
	\item the Coriolis effect
	\item the variation of gravitational acceleration with height
\end{enumerate}
\end{exercise}
...
\begin{enumerate}
	...
	\item In real code, you should avoid reimplementing common functions such as this.  Find a standard Python implementation for matrix transposition online and change your \shellcmd{transpose()} function to call it instead.
	\item Writing small functions that simply call libraries can be useful: if at a later date you found a better third party library, the changes required to your code are minimal.  What other benefits does this approach bring? % lets you write functions that are more meaningful to your code; convenient place to add logging/other hooks; easier to write automated tests
\end{enumerate}
\end{exercise}
...
\item You should see a list of variables.  Right click on `Mean sea level pressure' and choose \menu{Create Standard Plot}.  Accept the default options and choose \menu{Create}.  You should see a plot that looks like figure~\ref{fig:panoply}.
\end{enumerate}

\begin{figure}
	\includegraphics[width=\textwidth]{panoply.png}
...
You will be prompted to enter your password.
	\item To plot geographic data we must install a separate matplotlib component called `basemap'.  Download the most recent \shellcmd{.tar.gz} file from \url{http://sourceforge.net/projects/matplotlib/files/matplotlib-toolkits/}
...
Here we will present a small program that plots surface temperature on a Mercator projection of the world.
\begin{minted}{python}
from netCDF4 import Dataset
...
basemap = Basemap(projection='merc', llcrnrlat=-80, \
...
plt.show()
\end{minted}
This plots a map shown in figure~\ref{fig:netcdf}.

\begin{figure}
	\includegraphics[width=\textwidth]{netcdf.png}
	\caption{Visualisation of ERA-40 data with matplotlib}
	\label{fig:netcdf}
\end{figure}

There are a few points to mention about the code:
\begin{itemize}
	\item We load the NetCDF file into a \shellcmd{Dataset} structure which holds a dictionary of variables (see section~\ref{sec:dictionary} for details about the dictionary data structure).  The variable names match those seen in Panoply.
	\item We use matplotlib's \shellcmd{Basemap} to project (lat, lon) data onto (x, y) coordinates.  Refer to \url{http://matplotlib.org/basemap/users/mapsetup.html} for details of the available projections.
	\item The ERA-40 longitudes range from \SIrange{0}{360}{\degree}, but matplotlib expects longitudes in the range \SIrange{-180}{180}{\degree}.  We use the \shellcmd{shiftgrid()} function modify the data to account for this.
\end{itemize}
