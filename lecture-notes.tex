...
\item Possibly the most common operation on arrays is `iteration', in which the same code is executed for each element in an array.  We can iterate over an array using a \shellcmd{for} loop.
\item To print the first few multiples of ten, we could write
\begin{minted}{python}
for i in range(5):
	print(10**i)
\end{minted}
