...
\item Enter the command \shellcmd{sudo apt-get install ipython3 python3-tk}.  You will be prompted to enter your password.  Enter \keys{Y} when prompted to continue.
...
\begin{exercise}
\begin{enumerate}
\item Rewrite the `Blast off!' example on page~\pageref{blast-off} using a \shellcmd{for} loop
...
Implement a root finding function using the bisection method.  Use it to find an approximate solution to $x^5 + x + 1 = 0$ using $a = -1.0$, $b = 1.0$, $\epsilon = \num{1e-6}$ and a suitable maximum number of iterations.

\additional

Modify your root finding function to take $a$, $b$ and $\epsilon$ as arguments.  Make the maximum number of iterations an optional argument with a suitable default value.

\end{enumerate}
\end{exercise}

\TODO{rewrite the bisection root finder using a function reference}
